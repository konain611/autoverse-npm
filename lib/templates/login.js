...
export default function AutoverseDashboardLogin() {
  ...
  const AGENT_NAME = process.env.NEXT_PUBLIC_AUTOVERSE_AGENT_NAME || 'Autoverse';
  const USERNAME   = process.env.NEXT_PUBLIC_AUTOVERSE_USERNAME;
  const PASSWORD   = process.env.NEXT_PUBLIC_AUTOVERSE_PASSWORD;

  useEffect(() => {
    if (localStorage.getItem('autoverse_auth') === 'true') {
      router.replace('/autoverse-dashboard/home');
    }
  }, []);
  ...
  function handleLogin() {
    ...
    setTimeout(() => {
      if (user === USERNAME && pass === PASSWORD) {
        ...
        setError('');
        router.push('/autoverse-dashboard/home');
      } else {
