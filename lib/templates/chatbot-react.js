...
function getGeminiApiKey() {
  return (
    getPublicEnv('VITE_AUTOVERSE_GEMINI_API_KEY') ||
    getPublicEnv('REACT_APP_AUTOVERSE_GEMINI_API_KEY') ||
    getPublicEnv('NEXT_PUBLIC_AUTOVERSE_GEMINI_API_KEY') ||
    ''
  );
}
...
async function requestGeminiReply(messages, settings) {
  const apiKey = getGeminiApiKey();
  if (!apiKey) {
    throw new Error('Missing public Gemini API key. Re-run the CLI or add VITE_AUTOVERSE_GEMINI_API_KEY / REACT_APP_AUTOVERSE_GEMINI_API_KEY.');
  }

  const latest = messages.slice(-20);
  const contents = latest.map((message) => ({
    role: message.role === 'user' ? 'user' : 'model',
    parts: [{ text: message.text || '' }],
  }));
  const temperature = Math.min(Math.max(Number(settings.temperature) || 0.7, 0), 2);
  const maxOutputTokens = Math.min(Math.max(Number(settings.maxOutputTokens) || 512, 64), 2048);
  const systemInstruction = String(settings.systemInstruction || DEFAULT_SETTINGS.systemInstruction).slice(0, 4000);
  ...
  if (!response.ok) {
    ...
    throw new Error('Gemini request failed. ' + details.slice(0, 220));
  }
...
export default function AutoverseChatbot() {
  ...
  async function handleSend() {
    ...
    try {
      const reply = await requestGeminiReply(afterUserMessages, settings);
      const botMessage = { id: createId('msg'), role: 'bot', text: reply, createdAt: nowIso() };
      updateConversation((current) => current.map((conversation) => (
